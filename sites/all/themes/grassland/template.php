 ...
 */
function _grassland_load() {
  $themepath = drupal_get_path('theme', 'grassland');
 ...
 */
function grassland_html_head_alter(&$head_elements) {
 ...
 */
function grassland_breadcrumb($variables) {
...
function grassland_field($variables) {
  $output = '';
 ...
 */
function grassland_field__taxonomy_term_reference($variables) {
  $output = '';
 ...
 */
function grassland_links__topbar_main_menu($variables) {
  // We need to fetch the links ourselves because we need the entire tree.
  $links = menu_tree_output(menu_tree_all_data(variable_get('menu_main_links_source', 'main-menu')));
  $output = _grassland_links($links);
 ...
 * Formats links for Top Bar http://foundation.zurb.com/docs/components/top-bar.html
 */
function grassland_links__topbar_secondary_menu($variables) {
  // We need to fetch the links ourselves because we need the entire tree.
 ...
 *
 * @see grassland_links__system_main_menu()
 * @see grassland_links__system_secondary_menu()
 */
function _grassland_links($links) {
  $output = '';

  foreach (element_children($links) as $key) {
    $output .= _grassland_render_link($links[$key]);
  }
 ...
 *   A rendered list of links, with no <ul> or <ol> wrapper.
 *
 * @see _grassland_links()
 */
function _grassland_render_link($link) {
  $output = '';
  ...
  if (!empty($link['#href'])) {
    ...
    if (!isset($rendered_link)) {
      $rendered_link = theme('grassland_menu_link', array('link' => $link));
    }
    ...
    if (!empty($link['#below'])) {
      ...
      foreach ($link['#below'] as $sub_link) {
        if (!empty($sub_link['#href'])) {
          $sub_menu .= _grassland_render_link($sub_link);
        }
      }
 ...
 */
function theme_grassland_menu_link($variables) {
  $link = $variables['link'];
 ...
 */
function grassland_preprocess_block(&$variables) {
 ...
 */
function grassland_preprocess_field(&$variables) {
 ...
 */
function grassland_preprocess_html(&$variables) {
  global $language;
  ...
  //  $font = str_replace(' ', '+', theme_get_setting('grassland_font'));
  //  if (theme_get_setting('grassland_font')) {
 ...
 */
function grassland_preprocess_node(&$variables) {
 ...
 */
function grassland_preprocess_page(&$variables) {
  ...
  // Top bar.
  if ($variables['top_bar'] = theme_get_setting('grassland_top_bar_enable')) {
    $top_bar_classes = array();

    if (theme_get_setting('grassland_top_bar_grid')) {
      $top_bar_classes[] = 'contain-to-grid';
    }

    if (theme_get_setting('grassland_top_bar_sticky')) {
      $top_bar_classes[] = 'sticky';
    }

    if ($variables['top_bar'] == 2) {
      $top_bar_classes[] = 'hide-for-large';
    }

    $variables['top_bar_classes'] = implode(' ', $top_bar_classes);
    $variables['top_bar_menu_text'] = check_plain(theme_get_setting('grassland_top_bar_menu_text'));

    $top_bar_options = array();

    if (!theme_get_setting('grassland_top_bar_custom_back_text')) {
      $top_bar_options[] = 'custom_back_text:false';
    }

    if ($back_text = check_plain(theme_get_setting('grassland_top_bar_back_text'))) {
      if ($back_text !== 'Back') {
        $top_bar_options[] = "back_text:'{$back_text}'";
      }
    }

    if (!theme_get_setting('grassland_top_bar_is_hover')) {
      $top_bar_options[] = 'is_hover:false';
    }

    if (!theme_get_setting('grassland_top_bar_scrolltop')) {
  ...
  if ($variables['alt_header'] = ($variables['top_bar'] != 1)) {
    ...
    $variables['alt_header_classes'] = $variables['top_bar'] == 2 ? ' hide-for-medium hide-for-small large-menu' : '';
  }
  ...
  // Messages in modal.
  $variables['grassland_messages_modal'] = theme_get_setting('grassland_messages_modal');
  ...
  if(theme_get_setting('grassland_messages_modal')) {
    drupal_add_js(drupal_get_path('theme', 'grassland') . '/js/behavior/reveal.js');
  }
}
 ...
 */
function grassland_css_alter(&$css) {
  // Remove defaults.css file.
  unset($css[drupal_get_path('module', 'system') . '/system.menus.css']);

  // Remove Drupal core CSS.
  if (theme_get_setting('grassland_disable_core_css')) {
 ...
 */
function grassland_pager($variables) {
  $tags = $variables['tags'];
  ...
  if ($pager_total[$element] > 1) {
    ...
    if (theme_get_setting('grassland_pager_center')) {
 ...
 */
function grassland_theme() {
  $return = array();

  $return['grassland_reveal'] = array(
    'variables' => array(
    ...
    'function' => 'theme_grassland_reveal',
  );

  $return['grassland_menu_link'] = array(
    'variables' => array('link' => NULL),
    'function' => 'theme_grassland_menu_link',
  );
  return $return;
}
 ...
 *
 * @see theme_grassland_reveal()
 */
function _grassland_reveal($reveal = NULL) {
 ...
 *
 * @see grassland_theme()
 * @see grassland_preprocess_region()
 * @see _grassland_reveal()
 */
function theme_grassland_reveal($variables) {
  // Generate unique IDs.
  static $counter = 0;
 ...
 */
function _grassland_add_reveals() {
  $markup = '';

  // Retrieve reveal markup from static storage.
  foreach (_grassland_reveal() as $reveal) {
 ...
 */
function grassland_theme_registry_alter(&$theme_registry) {
  ...
  foreach ($entity_info as $entity => $info) {
    if (isset($entity_info[$entity]['fieldable']) && $entity_info[$entity]['fieldable']) {
      ...
      if (isset($theme_registry[$entity])) {
        $theme_registry[$entity]['preprocess functions'][] = 'grassland_entity_variables';
      }
    }
  }
  ...
  if (isset($theme_registry['file_entity'])) {
    $theme_registry['file_entity']['preprocess functions'][] = 'grassland_entity_variables';
  }
 ...
 *
 * @see grassland_theme_registry_alter()
 */
function grassland_entity_variables(&$vars) {
 ...
 */
function grassland_process_html_tag(&$vars) {
  if (theme_get_setting('grassland_html_tags')) {
    $el = &$vars['element'];
 ...
 * @return string
 */
function grassland_links__magic_button($variables) {
  ...
  if (count($variables['links']) > 1) {
    switch ($variables['type']) {
      case 'split':
        return grassland_links__split_button($variables);
        break;

      case 'dropdown':
      default:
        return grassland_links__dropdown_button($variables);
        break;
    }
  }
 ...
 */
function grassland_links__dropdown_button($variables) {
  if (empty($variables['attributes']['class'])) {
    $variables['attributes']['class'] = array();
  }
  ...
  $output = _grassland_links($variables['links']);
 ...
 */
function grassland_links__split_button($variables) {
  $links = $variables['links'];
